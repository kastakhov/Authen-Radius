...
my @nodes = ('127.0.0.1:1820', '127.0.0.2:1830', '[::1]:1820', '[::2]:1830');
...
$auth = Authen::Radius->new(Host => '[::3]:1840', NodeList => \@nodes, Secret => 'secret', Debug => 0);
ok($auth, 'object created');
ok(!$auth->get_active_node, 'Host must be from NodeList');

$auth = Authen::Radius->new(Host => '127.0.0.1:1820', NodeList => \@nodes, Secret => 'secret', Debug => 0);
ok($auth, 'object created');
is($auth->get_active_node, '127.0.0.1:1820', 'active node pre-set by Host option');

$auth = Authen::Radius->new(Host => '[::1]:1820', NodeList => \@nodes, Secret => 'secret', Debug => 0);
ok($auth, 'object created');
is($auth->get_active_node, '[::1]:1820', 'active node pre-set by Host option');
